...
const RemoveClanFormFields = ({ formData, onInputChange }: any) => {
...
const RemoveClan: React.FC<RemoveClanProps> = ({ isOpen, onClose }) => {
  ...
  const handleRemoveClanAction = async () => {
    ...
    if (!result.success) {
      throw new Error(result.error || 'Erro ao deletar clã');
    }
  ...
  const getConfirmDescription = useCallback(() => {
    return `ATENÇÃO: Tem absoluta certeza que deseja DELETAR PERMANENTEMENTE o clã "${selectedClan?.strName || 'ID: ' + formData.oidGuild}"?\n\nRazão: "${formData.reason}"\n\nEsta ação é IRREVERSÍVEL e o clã será excluído definitivamente.`;
  ...
  return (
    <ActionFormModal
      ...
      onClose={onClose}
      title="REMOVER CLÃ"
      confirmTitle="Confirmar Exclusão de Clã"
      confirmDescription={getConfirmDescription()}
      confirmActionText="Sim, Deletar Permanentemente"
      action={handleRemoveClanAction}
      formData={formData}
      onFormDataChange={setFormData}
      requiresPlayerValidation={false}
      showPlayerFields={false}
      customValidation={customValidation}
      customValidationMessage="Por favor, informe um ID de clã válido e a razão da exclusão."
    >
      <RemoveClanFormFields />
    </ActionFormModal>
  );
};
